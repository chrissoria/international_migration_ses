...
# GDP Data
```{r}
gdp <- read_excel(here("data", "gdp_per_capita.xlsx")) %>%
...
```{r}
e60 <- read_csv(here("data", "e60.csv")) %>%
...
write.csv(e60, here("e60.csv"))
```

# Infant Mortality Rates
```{r}
ifr_path <- here("data", "IMR")
...
counts <- read.csv(here("data", "hispanic_immigrant_count.csv")) %>%
...
write.csv(background_data, here("background_data.csv"))

# Clean up intermediate objects
rm(ifr_path, ifr_files, ifr_dfs, ifr_df, ifr_wide, gdp, e60, counts)
```

# Table 1: Hispanic Immigrants Ages 60+ Living in US (2016-2020), and Birth Country Demographic Indicators
# Note: Immigrant counts use original person weights (perwt), NOT age-standardized weights.
# This table shows raw population counts by country of origin.
```{r}
background_data <- read.csv(here("data", "hispanic_immigrant_count.csv"))
...
# Export to Excel
write_xlsx(background_table, here("tables", "table1.xlsx"))

# Export to LaTeX
tbl <- xtable(background_table,
              caption = "Table 1: Hispanic Immigrants Ages 60+ Living in US (2016-2020), and Birth Country Demographic Indicators",
              align = "ll|cccccc")
latex_code <- capture.output(print(tbl, type = "latex", caption.placement = "top",
                                   size = "\\small", include.rownames = FALSE))
writeLines(latex_code, here("tables", "r_tables", "background_table.tex"))
