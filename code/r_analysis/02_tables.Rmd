...
# Setup
```{r}
# Use here() for portable paths across machines
data_dir <- here("data")
plot_dir <- here("plots")
...
source(here("code", "r_analysis", "helpers", "table_functions.R"))
```

# Reading in Data
```{r}
# Load US data
us_df_2010 <- read_dta(here("data", acs_2010)) %>% add_us_derived_vars()
us_df_2020 <- read_dta(here("data", "US_2020_v100.dta")) %>%
  add_us_derived_vars() %>%
  mutate(
...
in_df <- read_dta(here("data", "CuDrMePrUs_10_12.dta")) %>%
  mutate(
...
# Table 2: Sociodemographic Comparison of Hispanics Ages 60+ in the U.S. by Birth Country (2016-20 ACS)
## Note: This table uses original person weights (perwt), NOT age-standardized weights.
## This shows descriptive statistics for the US Hispanic immigrant population.
```{r}
# Main countries
...
export_table(
  table2,
  excel_path = here("tables", "table2.xlsx"),
  latex_path = here("tables", "r_tables", "table2.tex"),
  caption = "Table 2: Sociodemographic Comparison of Hispanics Ages 60+ in the U.S. by Birth Country (2016-20 ACS)",
...
export_table(
  table11,
  excel_path = here("tables", "table11.xlsx"),
  latex_path = here("tables", "r_tables", "table11.tex"),
...
# Table 3: Sociodemographic Comparison of Hispanics Ages 60+ by Birth Country: Non-Migrants versus US Immigrants (~2010)
## Note: This table uses original person weights (perwt), NOT age-standardized weights.
```{r}
# Load international data with original weights (not age-standardized)
in_df_original <- read_dta(here("data", "CuDrMePrUs_10_12.dta")) %>%
  mutate(country_year = as_factor(country_year))

table3 <- create_intl_comparison_table(in_df_original, us_df_2010)
...
export_table(
  table3,
  excel_path = here("tables", "table3.xlsx"),
  latex_path = here("tables", "r_tables", "table3.tex"),
...
# Supplemental Table 3: Sociodemographic Comparison of Hispanics Ages 60+ in the U.S. by Birth Country (2008-10 ACS)
## Note: This table uses original person weights (perwt), NOT age-standardized weights.
```{r}
...
export_table(
  table_2010,
  excel_path = here("tables", "table6.xlsx"),
  latex_path = here("tables", "r_tables", "table1_2010.tex"),
  caption = "Supplemental Table 3: Sociodemographic Comparison of Hispanics Ages 60+ in the U.S. by Birth Country (2008-10 ACS)",
  align = "ll|ccccccccccc",
  tabular_override = "\\begin{tabular}{>{\\raggedright\\arraybackslash}p{3.2cm}|lcccccc|cccc}"
)
...
export_table(
  table12,
  excel_path = here("tables", "table12.xlsx"),
  latex_path = here("tables", "r_tables", "table12.tex"),
...
# Supplemental Table 1: Sociodemographic Comparison of Hispanics Ages 60+ in the U.S. by Birth Country (2016-20 ACS): Females
## Note: This table uses original person weights (perwt), NOT age-standardized weights.
```{r}
table_female <- create_sex_specific_table(us_df_2020, 2, "2020")
export_table(
  table_female,
  excel_path = here("tables", "table4.xlsx"),
  latex_path = here("tables", "r_tables", "table1_female.tex"),
  caption = "Supplemental Table 1: Sociodemographic Comparison of Hispanics Ages 60+ in the U.S. by Birth Country (2016-20 ACS): Females",
  align = "ll|ccccccccccc",
  tabular_override = "\\begin{tabular}{>{\\raggedright\\arraybackslash}p{3.2cm}|lcccccc|cccc}"
)
```

# Supplemental Table 2: Sociodemographic Comparison of Hispanics Ages 60+ in the U.S. by Birth Country (2016-20 ACS): Males
## Note: This table uses original person weights (perwt), NOT age-standardized weights.
```{r}
table_male <- create_sex_specific_table(us_df_2020, 1, "2020")
export_table(
  table_male,
  excel_path = here("tables", "table5.xlsx"),
  latex_path = here("tables", "r_tables", "table1_male.tex"),
  caption = "Supplemental Table 2: Sociodemographic Comparison of Hispanics Ages 60+ in the U.S. by Birth Country (2016-20 ACS): Males",
  align = "ll|ccccccccccc",
  tabular_override = "\\begin{tabular}{>{\\raggedright\\arraybackslash}p{3.2cm}|lcccccc|cccc}"
)
```

# Supplemental Table 5: Age-Standardized Sociodemographic Comparison of Hispanics Ages 60+ by Birth Country (~2010)
## Note: Age groups use original weights; all other variables use age-standardized weights.
## Reference population for standardization: US 2010 Census Hispanics 60+ (by sex).
```{r}
# Get age distribution with ORIGINAL weights (not standardized)
table_age_original <- create_intl_comparison_table(in_df_original, us_df_2010)

# Get all variables with STANDARDIZED weights
us_df_2010_standardized <- us_df_2010 %>%
  mutate(perwt = perwt_age_standardized)
table_standardized <- create_intl_comparison_table(in_df, us_df_2010_standardized)

# Replace age rows (rows 1-4 for female, rows 13-16 for male after removing headers) with original weights
# Age rows are: 60-69, 70-79, 80-89, 90 plus (rows 1-4 in each sex section)
table_standardized[1:4, ] <- table_age_original[1:4, ]
table_standardized[13:16, ] <- table_age_original[13:16, ]
...
export_table(
  table_standardized,
  excel_path = here("tables", "table8.xlsx"),
  latex_path = here("tables", "r_tables", "table2_standardized.tex"),
...
# Supplemental Table 6: Summary Statistics by Country and Sex, Comparing 2010 versus 2020
## Note: This table uses age-standardized weights (perwt_age_standardized) for valid cross-country comparisons.
```{r}
table_2010_2020 <- create_year_comparison_table(in_df, us_df_2010, us_df_2020)

export_table(
  table_2010_2020,
  excel_path = here("tables", "table9.xlsx"),
  latex_path = here("tables", "r_tables", "table2_2010_2020.tex"),
...
# Supplemental Table 7: Changing Educational Attainment of the 1930-1950 Birth Cohort due to Selection: 2010 versus 2020
## Note: This table uses original person weights (perwt), NOT age-standardized weights.
## Compares education levels for the same birth cohort observed in 2010 vs 2020 to show survivorship selection effects.
```{r}
...
export_table(
  table_edu_mexico,
  excel_path = here("tables", "table10.xlsx"),
  latex_path = here("tables", "r_tables", "table_prop_education_mexico_code.tex"),
  caption = "Supplemental Table 7: Changing Educational Attainment of the 1930-1950 Birth Cohort due to Selection: 2010 versus 2020",
  align = "ll|cccc"
)
```

# Supplemental Table 4: Sociodemographic Comparison of Hispanics Ages 60+ in the U.S. by Birth Country (2016-20 ACS): Migrated After Age 24
## Note: This table uses original person weights (perwt), NOT age-standardized weights.
```{r}
...
export_table(
  table7,
  excel_path = here("tables", "table7.xlsx"),
  latex_path = here("tables", "r_tables", "table1_25_plus.tex"),
  caption = "Supplemental Table 4: Sociodemographic Comparison of Hispanics Ages 60+ in the U.S. by Birth Country (2016-20 ACS): Migrated After Age 24",
  align = "ll|ccccccccccc",
  tabular_override = "\\begin{tabular}{>{\\raggedright\\arraybackslash}p{3.2cm}|lcccccc|cccc}"
)
```

# Supplemental Table 8: Alternative Education Comparison of Hispanics Ages 60+ in the U.S. by Birth Country (2016-20 ACS)
## Defining Completed Primary as 5+ Years of Education (Instead of 6+)
## Note: This table uses original person weights (perwt), NOT age-standardized weights.
```{r}
...
write_xlsx(alt_edu_tables, here("tables", "alt_education_table.xlsx"))
```
