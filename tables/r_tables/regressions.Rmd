...
if (current_dir == target_dir) {
  ...
  out_dir <- file.path("C:\Users\Ty\Documents\GitHub\international_migration_ses\tables")
...
results_all <- create_models_by_sex(us_df_2020)
results_male <- create_models_by_sex(us_df_2020, sex_filter = 1)
results_female <- create_models_by_sex(us_df_2020, sex_filter = 2)
```
## full multivariate
```{r}
...
for (dv in dvs) {
  m1 <- tidy(results_all[[dv]]) %>% mutate(model = "no_controls_all", n = nobs(results_all[[dv]]))
  m2 <- tidy(country_coefs[[dv]]) %>% mutate(model = "all_controls_all", n = nobs(results_all[[dv]]))
  m3 <- tidy(results_female[[dv]]) %>% mutate(model = "no_controls_female", n = nobs(results_all[[dv]]))
  m4 <- tidy(female_country_coefs[[dv]]) %>% mutate(model = "all_controls_female", n = nobs(results_all[[dv]]))
  m5 <- tidy(results_male[[dv]]) %>% mutate(model = "no_controls_male", n = nobs(results_all[[dv]]))
  m6 <- tidy(male_country_coefs[[dv]]) %>% mutate(model = "all_controls_male", n = nobs(results_all[[dv]]))
  ...
  write_xlsx(combined, 
    path = paste0(out_dir, "../regressions/", dv, "_models.xlsx"))
}
```
