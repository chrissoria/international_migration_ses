...
us_df_2010 <- read_dta(paste0(data_dir, acs_2010)) %>% 
  mutate(
    cognitive_difficulty = case_when(
      diffrem == 1 ~ 0,
      diffrem == 2 ~ 1,
      TRUE ~ NA_real_
    ),
    independent_living_difficulty = case_when(
      diffmob == 1 ~ 0,
      diffmob == 2 ~ 1,
      TRUE ~ NA_real_
    ),
    independent_care_difficulty = case_when(
      diffcare == 1 ~ 0,
      diffcare == 2 ~ 1,
      TRUE ~ NA_real_
    ),
...
table1<- us_df_2020 %>%
    ...
    summarise(
      ...
      mean_cog_difficulty = weighted.mean(cognitive_difficulty, perwt, na.rm = TRUE),
      mean_ind_live_difficulty = weighted.mean(independent_living_difficulty, perwt, na.rm = TRUE),
      N = n()
    ) %>%
...
rm(table1_code, adjustments, latex_code, adjust_latex_rows)
```
...
table1 <- table1 %>% 
  mutate(mean_educ_24_plus = table1_25_plus$mean_educ)
...
table2 <- table2 %>% 
  mutate(mean_educ_international = table2_international$mean_educ,
         mean_educ_2020 = table1$mean_educ,
         mean_lives_alone_international = table2_international$mean_lives_alone,
         mean_married_cohab_international = table2_international$mean_married_cohab)
