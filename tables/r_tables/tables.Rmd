...
table2_consolidated <- us_df %>%
    ...
    mutate(variable = recode(variable,
      ...
      mean_married_cohab = "Married/Cohabiting",
      mean_female = "Proportion Female",
      mean_english_speaker = "English Speakers",
      mean_is_citizen = "Proportion Citizen",
...
table2_consolidated <- table2_consolidated %>%
  ...
  add_row(Demographics = "Age Migrated", .before = 13) %>% 
  add_row(Demographics = "Migration Cohort", .before = 16)
...
latex_code[tabular_line_index] <- "\\begin{tabular}{>{\\raggedright\\arraybackslash}p{3cm}|cccccccc}"

# To align rows right
rows_to_align_right <- c("60 - 69", "70 - 79", "80 - 89", "Less than Primary", "Primary", "Secondary", "University", "Less than 15", "15 - 49", "Greater than 50", "Before 1965", "1965 - 1979", "1980 - 1999", "After 2000")
for (row in rows_to_align_right) {
  row_index <- grep(paste0("^\\s*", row), latex_code)
  if (length(row_index) > 0) {
    latex_code[row_index] <- gsub("^(\\s*)(.*?)(&.*)", 
                                  "\\1\\\\multicolumn{1}{>{\\\\raggedleft\\\\arraybackslash}p{3cm}|}{\\\\makebox[3cm][r]{\\2}}\\3", 
