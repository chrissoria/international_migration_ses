...
create_table_for_sex_international <- function(sex_value) {
  in_df %>%
    ...
    mutate(variable = recode(variable,
      ...
      mean_has_child_in_hh = "Child in the Household", 
...
table2_consolidated <- us_df_2020 %>%
    ...
    mutate(variable = recode(variable,
      ...
      mean_age_at_immigration_50plus = "Greater than 49",
      mean_years_in_us = "Years in US",
      mean_less_than_primary_completed = "Less than Primary",
      mean_primary_completed = "Primary",
      mean_secondary_completed = "Secondary",
      mean_university_completed = "University",
      mean_yrimm_before1965 = "Before 1966",
...
table2_consolidated <- table2_consolidated %>%
  ...
  add_row(Demographics = "Household", .before = 10) %>% 
  add_row(Demographics = "Age Migrated", .before = 15) %>% 
  add_row(Demographics = "Migration Cohort", .before = 20)
...
sink()
```
## proportion of age group that speaks english
```{r}
