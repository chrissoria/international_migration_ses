...
```{r}
# US
us_df_2010 <- read_dta(paste0(data_dir, census_2010))
...
background_table <- background_data %>% 
  select(bpld_string, GDPpercapita, e60, IMR_1950, IMR_2019) %>% 
  rename(Country = bpld_string,
         `GDP` = GDPpercapita,
...
```
# table 3, international and 2020
```{r}
create_table_for_sex_international <- function(sex_value) {
  in_df %>%
    filter(sex == sex_value) %>%
...
```
# proportion educated mexicans in those born in 1930-50
```{r}
in_df <- in_df %>% 
...
table_prop_education_mexico_us_2020_sex1 <- create_table_for_sex(us_df_2020, 1) %>% 
  select(mexico_2020)

table_prop_education_mexico_us_2020_sex2 <- create_table_for_sex(us_df_2020, 2) %>% 
  select(mexico_2020)
