...
e60 <- read_csv(paste0(data_dir, "e60.csv")) %>%
  filter(Location %in% c("Cuba", "Dominican Republic", "Puerto Rico", "Mexico"),
         Period == 2010,
         Indicator == "Life expectancy at 60 (years)") %>%
...
rm(ifr_2010, ifr_1950, ifr_df, ifr_dfs, gdp, e60)
```
## table on background features
```{r}
background_table <- background_data %>% 
  filter(e60_gender == "Both sexes") %>% 
...
table2_consolidated <- us_df_2020 %>%
    ...
    summarise(
      ...
      mean_age_at_immigration_15to23 = weighted.mean(age_at_immigration_15to23, perwt, na.rm = TRUE),
      mean_age_at_immigration_24to49 = weighted.mean(age_at_immigration_24to49, perwt, na.rm = TRUE),
    ...
    mutate(variable = recode(variable,
      ...
      mean_age_at_immigration_15to23 = "15 - 23",
      mean_age_at_immigration_24to49 = "24 - 49",
...
table2_consolidated_migrant <- us_df_2020 %>%
  ...
  filter(hispanic_migrant_status != "native-born not hispanic") %>% 
...
table2_consolidated_native_race <- us_df_2020 %>%
...
table2_consolidated <- table2_consolidated %>%
  ...
  select(variable, mexico, `puerto rico`, `dominican republic`, cuba,`foreign-born hispanic`, `foreign-born not hispanic`, `native-born hispanic`, `Black`, `White`, `Other`) %>% 
  rename(
    ...
    Cuba = cuba,
    `Hispanic` = `foreign-born hispanic`,
    `Non-Hispanic` = `foreign-born not hispanic`,
    `Native Hispanic ` = `native-born hispanic`
  ) %>% 
...
latex_code[tabular_line_index] <- "\\begin{tabular}{>{\\raggedright\\arraybackslash}p{3.2cm}|lccccc|cccc}"
...
adjustments <- list(
  ...
  list(rows = c("Married/Cohabiting"), width = 3.3)
)
...
variable_labels <- c(
  ...
  mean_english_speaker = "English Speakers",
  mean_is_citizen = "Citizen",
  mean_age_at_immigration = "at Immigration",
  mean_years_in_us = "Years in US",
...
create_table_for_race <- function(sex_value) {
  us_df_2020 %>%
    ...
    group_by(hispanic_migrant_status_race) %>%
    summarise(
      mean_= weighted.mean(age, perwt, na.rm = TRUE),
      mean_married_cohab = weighted.mean(married_cohab, perwt, na.rm = TRUE),
      mean_english_speaker = weighted.mean(english_speaker, perwt, na.rm = TRUE),
      mean_is_citizen = weighted.mean(is_citizen, perwt, na.rm = TRUE),
...
sink(paste0(out_dir, "r_tables/table3.tex"))
...
sink()
```
## proportion of group that speaks english
```{r}
...
```
## which migration cohort more likely to speak english
```{r}
result_list <- list()
