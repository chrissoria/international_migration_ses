...
for (reason in names(results_list)) {
  ...
  if (!is.null(results_list[[reason]])) {
    # Convert the model summary to a data frame using broom::tidy()
    model_summary <- tidy(results_list[[reason]])
    
    # Add the model summary to the excel_data list
...
# Check if excel_data has been populated
if (length(excel_data) > 0) {
  # Write all tables to an Excel file, each model in a separate sheet
